...
const Invoicetds = db.Invoicetds;
const InvoicetdsItems = db.InvoicetdsItems;
...
router.get("/getvendordata/:id", verifytoken, function(req, res, next) {
   const id = req.params.id;
    db.sequelize.query(
            "EXEC ARcustomer_list @LoginUserID = '"+id+"'", { replacements: ["active"], type: db.Sequelize.QueryTypes.SELECT }
        )
        .then((data) => {
            res.status(200).send({
                response_code: "200",
...
router.get("/getbookingnumber/:id/", verifytoken, function (req, res, next) {
    ...
    db.sequelize.query(
        "select InvoiceNumber from Invoices where CompanyName="+ id +"  group by InvoiceNumber ", { replacements: ["active"], type: db.Sequelize.QueryTypes.SELECT }
    )
        .then((data) => {
            res.status(200).send({
                response_code: "200",
...
router.post('/', verifytoken, async function(req, res, next) {
    ...
    try{
        const newCompany = await Invoicetds.create({
            ClientName:jsondata.company.Client_Id,
            InvoiceNumber:jsondata.invoiceno.InvoiceNumber,
        ...
        const response = CF.getStandardResponse(201, "Payment entry created successfully",{ id: newCompany.APE_id });
...
router.post('/invoicetds/list', verifytoken, async function (req, res, next) {
    ...
    try {
        for (var i = 0; i < jsondata.length; i++) {
            ...
            const newCompany = await InvoicetdsItems.create({
                APE_id: jsondata[i].invoiceID,
...
router.put('/update/:id', verifytoken, async function (req, res, next) {
    ...
    try {
      ...
      const updatedCompany = await Invoicetds.update(updatedCompanyData, {
        where: { APE_id: id }
      });
  ...
  router.put('/listupdate/:id', verifytoken, async function (req, res, next) {
    ...
    try {
        await db.sequelize.query(
            "DELETE FROM ArPaymentlineitems WHERE APE_id = :id",
            {
                replacements: { id: id },
...
router.get('/invoices/:id/', verifytoken, (req, res) => {
    ...
    SELECT
    SubTotal,
    COALESCE(GSTAmount, 0) + COALESCE(CGSTAmount, 0) + COALESCE(IGSTAmount, 0) AS GST,
    GrandTotal
FROM Invoices
...
router.get("/paymentgrid/:id/:role", verifytoken, function (req, res, next) {
    ...
    const role = req.params.role;
       ...
       db.sequelize.query(
           "EXEC ARPaymentEntry_Grid @LoginUserID='"+id+"'", { replacements: ["active"], type: db.Sequelize.QueryTypes.SELECT }
       )
...
router.get("/getupdate/:id", verifytoken, function (req, res, next) {
    const id = req.params.id;
    db.sequelize.query(
        "select * from ArPaymententry where APE_id=" + id + " ", { replacements: ["active"], type: db.Sequelize.QueryTypes.SELECT }
    )
        .then((booking) => {
            if (!booking) {
                ...
                db.sequelize.query(
                    "select * from ArPaymentlineitems where APE_id=" + id + " ", { replacements: ["active"], type: db.Sequelize.QueryTypes.SELECT }
                )
                    .then((items) => {
                        console.log(items);
                        res.status(200).send({
