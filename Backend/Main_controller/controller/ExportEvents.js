...
router.post('/Events', verifytoken, async function(req, res, next) {
    ...
    try {
            ...
            if (isValidDate(Actual)|| Actual === null) {
                ...
                .then((data) => {
                    if (data[0].Status === 0) {
                        ...
                        .then(() => {
                            sendEmailNotification(bookingnumber, containerId, Milestones,Actual,CustomerAddress,locationName,LinearBkgno,SealNumber,names,clientEmailId);
        ...
        for (var i = 0; i < jsondata.length; i++) {
            ...
            var sql = "INSERT INTO ExportMilestonesStatusList (containerNumber,milestones,EDT,ADT) VALUES (?,?,?,?)";
...
//                 return db.sequelize.query(
//                     "INSERT INTO ExportMilestonesStatusList (containerNumber, milestones, EDT, ADT) VALUES (?, ?, ?, ?)",
//                     { 
...
router.put('/items/:id', verifytoken, async function(req, res, next) {
    ...
    try {
        ...
        // Local formatter (keeps input as-is without UTC shift)
        ...
        // Process events (Eventslog check + insert)
        let eventPromises = jsondata.map(event => {
            let { Actual, Milestones, containerId } = event;
        ...
        // Delete old milestones for this container
        await db.sequelize.query(
            "DELETE FROM ExportMilestonesStatusList WHERE containerNumber = :id",
            { 
                replacements: { id: id }, 
        ...
        // Insert updated milestones (with correct datetime formatting)
        let milestonePromises = jsondata.map(event => {
...
router.get("/mile/:id", verifytoken, function(req, res, next) {
     const id = req.params.id;
    db.sequelize.query(
            "select * from ExportMilestonesStatusList where containerNumber='"+id+"' order by milestones asc", { replacements: ["active"], type: db.Sequelize.QueryTypes.SELECT }
        )
        .then((data) => {
            res.status(200).send({
                response_code: "200",
...
router.get("/admin/", verifytoken, function(req, res, next) {
     const id = req.params.id;
    db.sequelize.query(
            "select Email from UserLoginDetails where User_Roleid=0", { replacements: ["active"], type: db.Sequelize.QueryTypes.SELECT }
        )
        .then((data) => {
            res.status(200).send({
                response_code: "200",
...
router.get("/client/:id", verifytoken, function(req, res, next) {
     const id = req.params.id;
    db.sequelize.query(
            "select Email from SKY_CreateClients where Client_Id="+id+"", { replacements: ["active"], type: db.Sequelize.QueryTypes.SELECT }
        )
        .then((data) => {
            res.status(200).send({
                response_code: "200",
...
router.get("/container/:id", verifytoken, function(req, res, next) {
    const id = req.params.id;
    db.sequelize.query(
            "select LItem_id,containernumber from EB_ItemDetails where EB_id="+id+"", { replacements: ["active"], type: db.Sequelize.QueryTypes.SELECT }
        )
        .then((data) => {
            res.status(200).send({
                response_code: "200",
