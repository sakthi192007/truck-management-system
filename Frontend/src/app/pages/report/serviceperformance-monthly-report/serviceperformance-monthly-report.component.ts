...
@Component({
  selector: 'app-serviceperformance-monthly-report',
  templateUrl: './serviceperformance-monthly-report.component.html',
  styleUrls: ['./serviceperformance-monthly-report.component.css']
})
export class ServiceperformanceMonthlyReportComponent  {
  currentuser: any;
  userId: any;
  RoleId: any;
  monthlygrid!: any[];
  ...
  get filteredData() {
    if (!this.searchTerm && !this.fromDate && !this.toDate) {
        return this.monthlygrid;
    }

    return this.monthlygrid.filter(bird => {
...
// }
  getallgrid(): void {
    this.APIServices.monthlygrid(this.userId).subscribe(
      (value) => {
        this.tablegridviews = value;
        this.monthlygrid = this.tablegridviews['data'];
        console.log(this.monthlygrid);
        this.dtTrigger.next(this.monthlygrid);
      },
      (error) => {
 ...
 exportToExcel() {
    ...
    // Save the file
    this.saveAsExcelFile(excelBuffer, 'VolumeExportData');
  }
