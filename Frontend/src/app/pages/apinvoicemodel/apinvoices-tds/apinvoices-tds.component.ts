...
@Component({
  selector: 'app-apinvoices-tds',
  templateUrl: './apinvoices-tds.component.html',
  styleUrl: './apinvoices-tds.component.css'
})
export class APInvoicesTDSComponent {
  ...
  constructor(
    ...
    private APIServies: ApinvoiceService,
...
loadLineItems(){
  ...
  const id = String(this.roleconcept[0].ClientName);
    ...
    const selectedType = this.typedata.find(
      ...
      this.apkey = this.roleconcept[0].AE_id;
      ...
      this.APIServies.dropbookingnumbers(this.roleconcept[0].ClientName).subscribe(value => {
        ...
        const InvoiceReference = String(this.roleconcept[0].InvoiceNumber);

    const selectedCustomer = this.getinvoiceNumber.find(
      c => String(c.InvoiceReference) === InvoiceReference
    );

    if (selectedCustomer) {
      setTimeout(() => {  
        this.invoicetdsForm.get('invoiceno')?.setValue(selectedCustomer);
      }, 50);
    }
      ...
      this.InvoiceID = this.roleconcept[0].AE_id;
      this.AE_id = this.roleconcept[0].AE_id;
 ...
 checkComplete();
  ...
  onCompanyName(event: any): void {
    this.selectCompanyName = event.value.CD_ID;
    this.APIServies.dropbookingnumbers(this.selectCompanyName).subscribe(value => {
      this.vendorselect = value;
      this.getinvoiceNumber = this.vendorselect['data'];
      this.book = this.getinvoiceNumber[0].InvoiceReference;
  ...
  onExportBookingChange(selectedItem: any): void {
    ...
    const selectedInvoiceNumber = selectedItem.value.InvoiceReference; 
  ...
  submit(itemsdata: any) {
    ...
    if (!itemsdata.length) {
        this.notifyService.showWarning("Please add data item details", "Payment entry");
      return;
    }
    ...
    this.APIServies.insertentrypayment(formvalues).subscribe(response => {
      if (response && response.data) {
        ...
        this.APIServies.insertlistpayment(updatedItems).subscribe(() => {
          ...
          this.router.navigate(['/APPaymentgrid']);
        }, error => {
